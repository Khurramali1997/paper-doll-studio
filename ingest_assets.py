...
def parse_config():
    ...
    # Extract JSON between "const DOLL_CONFIG = ", "window.DOLL_CONFIG = " and the final ";"
    match = re.search(r'(?:const|window\.)DOLL_CONFIG\s*=\s*({.*});', content, re.DOTALL)
    if not match:
...
def write_config(config_data):
    """Write config data back to doll_config.js and project.json."""
    # Write JS config
    ...
    # Write canonical project.json
    project_data = {
        "version": 1,
        "canvas": config_data.get("canvas", {"width": 768, "height": 768}),
        "layers": config_data.get("layers", []),
        "wardrobe": config_data.get("wardrobe", {}),
        "defaults": config_data.get("defaults", {
            "theme": "dark", "zoom": 1.0, "showGrid": True, "offsets": {}
        }),
        "meta": {
            "name": "Paper Doll Project",
            "characterName": "Character",
            "sourceFiles": ["character.psd"]
        }
    }
